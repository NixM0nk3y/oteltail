...
func setupArguments() {
	...
	parseKinesisCw := os.Getenv("PARSE_KINESIS_CWLOGS")
	parseKinesisCwLogs = false
	if strings.EqualFold(parseKinesisCw, "true") {
		parseKinesisCwLogs = true
	}
...
func handler(ctx context.Context, ev map[string]interface{}) error {
	...
	case *events.KinesisEvent:
		if parseKinesisCwLogs {
			err = processKinesisCwEvent(ctx, evt, pClient)
		} else {
